...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    if ($checkStmt->rowCount() > 0) {
        ...
        $_SESSION['name'] = $name;
        $success = "Trainer profile updated successfully!";
    }
}
...
// 1. Fetch current trainer data
$stmt = $pdo->prepare("SELECT * FROM users WHERE id = ?");
$stmt->execute([$user_id]);
$user = $stmt->fetch();

// 2. Fetch Trainer Stats (Plans Built & Classes Instructed)
$statStmt = $pdo->prepare("SELECT COUNT(*) as total_plans FROM workout_plans WHERE trainer_id = ?");
...
<div class="container-fluid py-4">
    <div class="row mb-4">
        <div class="col-12">
            <h2 class="fw-bold"><i class="bi bi-person-badge"></i> Trainer Profile</h2>
            <hr>
        </div>
    </div>
        ...
        <div class="col-md-4 mb-4">
            <div class="card shadow-sm border-0 bg-white mb-4">
                <div class="card-body text-center py-5">
                    ...
                    <h4 class="fw-bold mb-1">Coach <?= htmlspecialchars($user['name']) ?></h4>
                    <p class="text-primary fw-bold small text-uppercase mb-2"><?= htmlspecialchars($user['title'] ?? 'Certified Trainer') ?></p>
            ...
            <div class="card shadow-sm border-primary bg-white">
                <div class="card-header bg-primary text-white fw-bold"><i class="bi bi-trophy"></i> Coaching Impact</div>
                <div class="card-body text-center py-4">
                    <h1 class="display-4 fw-bold text-primary mb-0"><?= $stats['total_plans'] ?></h1>
                    <p class="text-muted text-uppercase small tracking-wide fw-bold">Workout Plans Built</p>
                </div>
            </div>
        </div>
                    ...
                    <form method="POST">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12 mb-3">
                                ...
                                <input type="text" name="title" class="form-control" value="<?= htmlspecialchars($user['title'] ?? '') ?>" placeholder="e.g. Head Coach, Strength Specialist">
                            </div>
                        </div>
